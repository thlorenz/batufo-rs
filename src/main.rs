...
impl GameState {
    fn new(ctx: &mut Context, arena: Arena, game_props: GameProps) -> GameResult<GameState> {
        ...
        let floor_view = init_floor_view(ctx, &arena, game_props.render.tile_size)?;
        let grid_view = GridView::new(
            ctx,
            arena.ncols,
            arena.nrows,
            game_props.render.tile_size,
            game_props.colors.grid_color(),
        )?;
        let player_view = PlayerView::new(
            game_props.colors.player_hit_tile_color(),
            game_props.render.tile_size,
        );

        let cameras = Cameras::new(
            game_props.cameras.platform_lerp,
            game_props.cameras.planets_front_lerp,
        );
        let player = Player::new(arena.player, game_props.render.tile_size);
...
impl event::EventHandler for GameState {
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        while timer::check_update_time(ctx, gp.physics.simulation_fps) {
            self.player.update(gp.physics.delta_time);
            self.cameras.update(
                gp.physics.delta_time,
                self.player
                    .tile_position
                    .to_world_point(gp.render.tile_size),
                &win_size.into(),
            );
        }
        Ok(())
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        let gp = &self.game_props;
        graphics::clear(ctx, gp.colors.background());
    ...
    fn key_down_event(
        ...
        match keycode {
            KeyCode::W => self
                .player
                .accelerate([0.0, -gp.physics.thrust_acceleration]),
            KeyCode::D => self
                .player
                .accelerate([gp.physics.thrust_acceleration, 0.0]),
            KeyCode::S => self
                .player
                .accelerate([0.0, gp.physics.thrust_acceleration]),
            KeyCode::A => self
...
pub fn main() -> GameResult {
    ...
    let game_props = GameProps::load_conf()?;
